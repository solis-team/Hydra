...
class FilePathBuilder:
    ...
    @staticmethod
    def repo_windows_path(repo_base_dir, repo, window_size, slice_size):
        out_path = os.path.join(f'cache/{repo_base_dir}', 'window/repos', f'{repo}_ws{window_size}_slice{slice_size}.pkl')
        FilePathBuilder.make_needed_dir(out_path)
        return out_path

    
    @staticmethod
    def search_first_window_path(repo_base_dir, mode, repo, window_size):
        # mode includes gt and s-g
        out_path = os.path.join(f'cache/{repo_base_dir}/window/{mode}', f'{repo}_ws{window_size}.pkl')
        FilePathBuilder.make_needed_dir(out_path)
        return out_path
...
class Tools:
    ...
    @staticmethod
    def iterate_repository(repo, repo_base_dir='test-apps/test-apps'):
        base_dir = repo_base_dir
        patterns = [
            os.path.join(f'{base_dir}/{repo}', "**", "*.py"),
        ]
        files = []
